...
export default function FileContentEditor() {
  ...
  if (!file || file.type !== "file") {
    return (
      <div className="text-gray-500 text-sm">Select a file to edit content</div>
    );
  }

  return (
    <textarea
      className="w-full h-full bg-gray-900 text-white p-3 font-mono text-sm border border-gray-700 rounded outline-none"
      value={file.content || ""}
      onChange={(e) =>
        dispatch(
          updateFileContent({
